...
- [x] Prerequisites (Go 1.25+, Docker, certificates, network access)
- [x] Quick start with Docker (Mode A — pre-built image)
- [x] mTLS certificate enrollment via `chaperone.Enroll()` public API
  - [x] Document `EnrollConfig` struct and `EnrollResult` fields
  - [x] Show wiring `enroll` subcommand in Distributor's `main.go`
  - [x] Example: generate CSR, submit to CA, receive signed cert
- [x] Development certificates (`make gencerts` for testing)
- [x] Configuration file setup (copy from `configs/config.example.yaml`)
- [x] Docker deployment operations
  - [x] Running the container (port mapping: 8443 for proxy, 9090 for admin)
  - [x] Volume mounts for certificates and configuration
  - [x] Health checking without shell (distroless image has no curl/wget)
  - [x] Kubernetes liveness/readiness probe example (`httpGet` on admin port)
  - [x] Host-side verification: `curl -s http://localhost:9090/_ops/health`
  - [x] Production hardening notes (non-root, distroless, read-only rootfs)
- [x] Verification steps (health check, version, test proxy request)
- [x] Troubleshooting link

### Configuration Reference (`docs/configuration.md`)
- [x] File location resolution order (`-config` flag → `CHAPERONE_CONFIG` env → `./config.yaml`)
- [x] All config sections documented with types, defaults, env var overrides
- [x] Server section (addr, admin_addr, shutdown_timeout, TLS)
- [x] Upstream section (header_prefix, trace_header, allow_list, timeouts)
- [x] Observability section (log_level, enable_profiling, sensitive_headers)
- [x] Allow-list glob pattern syntax (`*` vs `**`) with examples
- [x] Environment variable override pattern (`CHAPERONE_<SECTION>_<KEY>`)
- [x] Timeout tuning guidance (connect, read, write, idle, keep_alive, plugin)
- [x] Complete annotated example (reference `configs/config.example.yaml`)

### Plugin Developer Guide (`docs/plugin-development.md`) — **Centerpiece**
- [x] Architecture overview: what is a plugin, why static recompilation (ADR-001 summary)
- [x] The `sdk.Plugin` interface: `CredentialProvider` + `CertificateSigner` + `ResponseModifier`
- [x] TransactionContext and Credential types explained
- [x] Fast Path vs Slow Path credential strategies with examples
- [x] ResponseAction and when to use `SkipErrorNormalization`
- [x] **Method 1: "Own Repo" (Recommended)**
  - [x] Directory layout (`go.mod`, `main.go`, `plugins/`)
  - [x] `go.mod` with `require` for both modules
  - [x] Pre-publication note: `replace` directives for local snapshot
  - [x] Post-publication note: remove `replace`, use versioned `require`
  - [x] Minimal `main.go` using `chaperone.Run()` with options
  - [x] Adding enrollment support: `chaperone.Enroll()` in subcommand routing
  - [x] Available `chaperone.Option` values (`WithConfigPath`, `WithVersion`, `WithBuildInfo`, `WithLogOutput`)
  - [x] Building: `go build -o my-proxy .`
  - [x] Docker deployment: Dockerfile template for Distributor binary
  - [x] Docker runtime: running, health checking, volume mounts, production hardening
- [x] **Method 2: "Fork/Extend" (Simpler alternative)**
  - [x] When to use (quick start, simple deployments)
  - [x] Clone repo, add plugin under `plugins/`, modify `cmd/chaperone/main.go`
  - [x] Build with `make build`
  - [x] Tradeoffs vs Own Repo (merge conflicts on upgrade, coupled versioning)
- [x] Testing your plugin
  - [x] Unit testing with standard Go `testing`
  - [x] Compliance suite: `compliance.VerifyContract(t, plugin)`
  - [x] Integration testing tips (mock TransactionContext, use httptest)
- [x] Common patterns: Bearer token, API key, OAuth2 refresh, Vault lookup
- [x] Reference plugin walkthrough (`plugins/reference/reference.go`)

### Troubleshooting (`docs/troubleshooting.md`)
- [x] Common startup errors (missing config, TLS cert errors, port conflicts)
- [x] mTLS issues (macOS LibreSSL + ECDSA workaround, cert chain validation)
- [x] Allow-list denials (403 responses, glob pattern debugging)
- [x] Plugin errors (timeout, panic recovery, credential format)
- [x] Docker-specific issues (permissions, networking, image size)
- [x] Diagnostic tools (health endpoint, version endpoint, pprof, metrics)

### Documentation Index (`docs/README.md`)
- [x] Overview and audience
- [x] Quick navigation to each guide
- [x] No references to `.github/` directory or its contents

### Root README (`README.md`)
- [x] Add links to docs/ guides
- [x] Ensure "Building Custom Binaries" section aligns with plugin-development.md
- [x] Ensure Configuration Reference section aligns with docs/configuration.md
- [x] Verify all code examples match actual SDK interfaces

### General
- [x] All docs in `docs/` directory
- [x] Clear, concise language targeting a Go developer audience
- [x] All Go code examples compile against current SDK interfaces
- [x] Copy-pasteable shell commands
- [x] No references to files in `.github/`
...
- [x] `docs/plugin-development.md` - Plugin developer guide
- [x] `docs/troubleshooting.md` - Troubleshooting

### Modify
- [x] `docs/README.md` - Update to be a documentation index (remove `.github/` references)
- [x] `README.md` - Add links to `docs/` guides, verify code examples match SDK
