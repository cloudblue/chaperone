...
describe("useInstanceForm", () => {
	...
	describe("initialization", () => {
		...
		it("populates from existing instance for editing", () => {
			const store = mockStore();
			const { result } = withSetup(() => useInstanceForm(store, { id: 5, name: "proxy-1", address: "10.0.0.1:9090" }));
	...
	describe("handleTest", () => {
		...
		it("sets error testResult on network failure", async () => {
			const store = mockStore({ testConnection: vi.fn().mockRejectedValue(new Error("network")) });
			const { result } = withSetup(() => useInstanceForm(store));
			result.address.value = "10.0.0.1:9090";
			await result.handleTest();
			expect(result.testResult.value).toEqual({ ok: false, error: "Failed to test connection" });
		});

		it("manages testing flag during the request", async () => {
			let resolve;
			const store = mockStore({ testConnection: vi.fn().mockReturnValue(new Promise((r) => (resolve = r))) });
			const { result } = withSetup(() => useInstanceForm(store));
			result.address.value = "10.0.0.1:9090";
	...
	describe("handleSubmit", () => {
		...
		it("calls createInstance for new instance and returns true", async () => {
			...
			const ok = await result.handleSubmit();
			expect(ok).toBe(true);
			expect(store.createInstance).toHaveBeenCalledWith("proxy-1", "10.0.0.1:9090");
		});
		...
		it("calls updateInstance for existing instance and returns true", async () => {
			...
			const ok = await result.handleSubmit();
			expect(ok).toBe(true);
			expect(store.updateInstance).toHaveBeenCalledWith(5, "proxy-updated", "10.0.0.1:9090");
		});

		it("sets address error on API failure and returns false", async () => {
			const store = mockStore({ createInstance: vi.fn().mockRejectedValue(new Error("Address already registered")) });
			const { result } = withSetup(() => useInstanceForm(store));
			result.name.value = "proxy-1";
			result.address.value = "10.0.0.1:9090";
			const ok = await result.handleSubmit();
		...
		it("manages saving flag during the request", async () => {
			let resolve;
			const store = mockStore({ createInstance: vi.fn().mockReturnValue(new Promise((r) => (resolve = r))) });
			const { result } = withSetup(() => useInstanceForm(store));
			result.name.value = "proxy-1";
			result.address.value = "10.0.0.1:9090";
