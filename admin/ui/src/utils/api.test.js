...
describe("api client", () => {
	...
	function mockFetch(status, body, { json = true } = {}) {
		const res = {
			...
			status,
			json: json ? vi.fn().mockResolvedValue(body) : vi.fn().mockRejectedValue(new Error("not json")),
		};
	...
	describe("get", () => {
		...
		it("throws ApiError with server message on failure", async () => {
			mockFetch(404, { error: { code: "INSTANCE_NOT_FOUND", message: "No instance with ID 42" } });
	...
	describe("post", () => {
		it("sends JSON body and returns parsed response", async () => {
			...
			const result = await post("/api/instances", { name: "proxy-1", address: "10.0.0.1:9090" });
			expect(result).toEqual({ id: 1, name: "proxy-1" });

			const [url, opts] = globalThis.fetch.mock.calls[0];
			expect(url).toBe("/api/instances");
			expect(opts.method).toBe("POST");
			expect(JSON.parse(opts.body)).toEqual({ name: "proxy-1", address: "10.0.0.1:9090" });
		});

		it("throws ApiError with server message on conflict", async () => {
			mockFetch(409, { error: { code: "DUPLICATE_ADDRESS", message: "Address already registered" } });
			const err = await post("/api/instances", { name: "x", address: "1:2" }).catch((e) => e);
			expect(err).toBeInstanceOf(ApiError);
