...
* **Decision:** We selected a **Split Module Strategy** (`proxy-core` vs `proxy-sdk`).
...
1.  **Module Separation (Build Stability):**
    * The project is split into two Go modules: `github.com/connect/proxy-sdk` (Interfaces) and `github.com/connect/proxy-core` (Runtime).
    * The SDK version is decoupled from the Core. Distributors can safely upgrade the `proxy-core` to get security patches without modifying their plugin code, as long as the SDK Major Version (`v1`) remains stable.
...
*(Note: This is a first draft of the interface defined in `proxy-sdk` but the final version of the MVP may differ.)*
...
type TransactionContext struct {
    ...
    Data           map[string]interface{} // Unmarshaled from {HeaderPrefix}-Context-Data (JSON)
}
...
type Plugin interface {
    CredentialProvider
    CertificateSigner
    ResponseModifier // Optional: Can be nil or no-op
}
...
// 2. CERTIFICATE SIGNING INTERFACE (New in v1.1)
type CertificateSigner interface {
    ...
    SignCSR(ctx context.Context, csrPEM []byte) (crtPEM []byte, error)
}
...
**Example Implementation (Distributor Logic):**

*(Note: These examples can give an idea of ​​how the plugins would be implemented by the Distributor, but they should not be taken as a reference since the interface defined in `proxy-sdk` could change during the development of the MVP.)*
...
```go
func (p MyPlugin) ModifyResponse(ctx context.Context, tx TransactionContext, resp *http.Response) error {
...
* **Requirement:** 80%+ Code Coverage for `proxy-core`.
...
* **Solution:** The `proxy-sdk` includes a **Compliance Test Kit**.
