...
- Sensitive headers (`Authorization`, `Cookie`, etc.) stripped
...
When enabled, Go's standard `pprof` endpoints are registered:
...
> **Warning:** Profiling endpoints should only be enabled in development or
> staging environments. They are disabled by default for security.
