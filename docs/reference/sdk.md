...
A Plugin must implement both sub-interfaces:
[CredentialProvider](#credentialprovider) and
[ResponseModifier](#responsemodifier). For features you don't
need, provide minimal stub implementations (see
[Plugin Development Guide](../guides/plugin-development.md)).

> **Note:** `CertificateSigner` was removed from the `Plugin` interface in
> this release. Certificate rotation is now driven by Connect over the
> existing mTLS connection via `POST /_ops/renew/prepare` and
> `POST /_ops/renew/install`. Plugins no longer need to implement `SignCSR`.
> See the [HTTP API Reference](http-api.md#certificate-renewal-endpoints-traffic-port-only)
> for details.
...
### CertificateSigner (Deprecated)

> **Deprecated.** `CertificateSigner` is no longer part of the `Plugin`
> interface. Certificate rotation is now orchestrated by Connect over mTLS
> using the `/_ops/renew/prepare` and `/_ops/renew/install` endpoints.
> The `CertificateSigner` interface is retained in the SDK for one release
> cycle to allow existing plugins to compile without modification, but
> `SignCSR` will never be called by the Core.

```go
// Deprecated: rotation is now Connect-driven. This interface will be removed in a future release.
