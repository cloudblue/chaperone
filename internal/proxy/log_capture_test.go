...
// captureLogsAt is like captureLogs but with explicit slog.HandlerOptions —
// used by tests that need to capture debug-level output.
func captureLogsAt(t *testing.T, opts *slog.HandlerOptions) func() string {
	t.Helper()
	prev := slog.Default()
	sb := &syncBuffer{}
	slog.SetDefault(slog.New(slog.NewJSONHandler(sb, opts)))
	t.Cleanup(func() { slog.SetDefault(prev) })
	return sb.String
}
